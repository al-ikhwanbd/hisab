...
async function checkAdmin(){if(!sb)return;const {data:{session}}=await sb.auth.getSession();adminUser=session?.user||null;if(!adminUser){q('loginBox').hidden=false;q('adminBox').hidden=true;return}const {data,error}=await sb.from('admin_users').select('user_id').eq('user_id',adminUser.id).maybeSingle();if(error||!data){q('loginBox').hidden=false;q('adminBox').hidden=true;q('loginMsg').textContent='এই অ্যাকাউন্টে অ্যাডমিন অনুমতি নেই।';return}q('loginBox').hidden=true;q('adminBox').hidden=false;q('adminUser').textContent=adminUser.email||'Admin';if(q('currentAdminEmail'))q('currentAdminEmail').textContent=adminUser.email||'';renderAdminData()}
...
async function sendPasswordReset(){if(!sb){showMessage('Supabase configuration পাওয়া যায়নি।',false,'forgotMsg');return}const email=(q('forgotEmail').value||q('adminEmail').value||'').trim();if(!email){showMessage('আগে অ্যাডমিন ইমেইল লিখুন।',false,'forgotMsg');return}showMessage('রিসেট লিংক পাঠানো হচ্ছে...',true,'forgotMsg');const redirectTo=new URL(location.href);redirectTo.hash='admin';const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:redirectTo.toString()});if(error){showMessage(error.message,false,'forgotMsg');return}showMessage('রিসেট লিংক আপনার ইমেইলে পাঠানো হয়েছে। ইমেইলের লিংকে ক্লিক করে নতুন পাসওয়ার্ড সেট করুন।',true,'forgotMsg')}
async function changeAdminEmail(){if(!sb||!adminUser){showMessage('আগে অ্যাডমিন হিসেবে লগইন করুন।',false,'accountMsg');return}const email=q('newAdminEmail').value.trim();if(!email){showMessage('নতুন ইমেইল লিখুন।',false,'accountMsg');return}if(email.toLowerCase()===(adminUser.email||'').toLowerCase()){showMessage('নতুন ইমেইলটি বর্তমান ইমেইলের মতোই।',false,'accountMsg');return}showMessage('ইমেইল পরিবর্তন করা হচ্ছে...',true,'accountMsg');const {data,error}=await sb.auth.updateUser({email});if(error){showMessage(error.message,false,'accountMsg');return}if(data?.user?.email&&data.user.email.toLowerCase()===email.toLowerCase()){adminUser=data.user;if(q('currentAdminEmail'))q('currentAdminEmail').textContent=data.user.email;showMessage('অ্যাডমিন ইমেইল পরিবর্তন হয়েছে।',true,'accountMsg')}else{showMessage('নতুন ইমেইলে একটি নিশ্চিতকরণ লিংক পাঠানো হয়েছে। লিংকটি নিশ্চিত করার পর নতুন ইমেইল কার্যকর হবে।',true,'accountMsg')}q('newAdminEmail').value=''}
function showPasswordReset(){if(q('loginBox'))q('loginBox').hidden=true;if(q('adminBox'))q('adminBox').hidden=true;if(q('passwordResetBox'))q('passwordResetBox').hidden=false}
async function resetPassword(){if(!sb)return;const p=q('resetPassword').value,p2=q('resetPasswordConfirm').value;if(!p){showMessage('নতুন পাসওয়ার্ড লিখুন।',false,'resetMsg');return}if(p.length<6){showMessage('পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে।',false,'resetMsg');return}if(p!==p2){showMessage('দুইটি পাসওয়ার্ড মিলছে না।',false,'resetMsg');return}showMessage('পাসওয়ার্ড পরিবর্তন হচ্ছে...',true,'resetMsg');const {error}=await sb.auth.updateUser({password:p});if(error){showMessage(error.message,false,'resetMsg');return}showMessage('পাসওয়ার্ড সফলভাবে পরিবর্তন হয়েছে। এখন নতুন পাসওয়ার্ড দিয়ে লগইন করুন।',true,'resetMsg');q('resetPassword').value='';q('resetPasswordConfirm').value='';setTimeout(()=>{if(q('passwordResetBox'))q('passwordResetBox').hidden=true;if(q('loginBox'))q('loginBox').hidden=false;location.hash='admin'},1200)}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  q('loginBtn').addEventListener('click',login);q('logoutBtn').addEventListener('click',logout);q('forgotPasswordBtn').addEventListener('click',()=>{q('forgotBox').hidden=!q('forgotBox').hidden;if(!q('forgotBox').hidden&&q('forgotEmail')&&!q('forgotEmail').value)q('forgotEmail').value=q('adminEmail').value.trim()});q('sendResetBtn').addEventListener('click',sendPasswordReset);q('changeAdminEmailBtn').addEventListener('click',changeAdminEmail);q('resetPasswordBtn').addEventListener('click',resetPassword);
  if(sb)sb.auth.onAuthStateChange((event)=>{if(event==='PASSWORD_RECOVERY'){showPasswordReset();location.hash='admin'}});
