...
function reportColumnWidths(count){
  const serialWidth=4;
  const nameLength=Math.max(
    4,
    ...members.map((m,i)=>String(m.name||'').trim().length || 4)
  );
  const nameWidth=Math.min(22,Math.max(16,16+(nameLength-12)*0.25));
  const equalWidth=(100-serialWidth-nameWidth)/count;
  return {serialWidth,nameWidth,equalWidth};
}
function downloadAllMembersReport(){
  ...
  const widths=reportColumnWidths(months.length+2);
